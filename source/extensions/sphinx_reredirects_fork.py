# A fork of sphinx-reredirects: https://gitlab.com/documatt/sphinx-reredirects
#
# We forked it because the current version of the package does not support the `html_file_suffix` parameter, which we
# use to strip URLs of the default `.html` suffix.
#
# See merge request: https://gitlab.com/documatt/sphinx-reredirects/-/merge_requests/2
    ...
    def create_redirects(self, to_be_redirected: Mapping[str, str]):
        ...
        for doc, target in to_be_redirected.items():
            redirect_file_abs = Path(
                self.app.outdir).joinpath(doc).with_suffix(suffix)
