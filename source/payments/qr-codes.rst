...
At Mollie, we support 2 types of payment QR codes: The payment specific QR-codes and the payment link QR codes. 

#. A payment method specific QR code is created by Mollie and is following the same expiry pattern like the :doc:`basic payment flow </payments/accepting-payments#the-basic-payment-flow>`. 
   This QR code expires at the same time the created payment expires. This can be created via the :doc:`payments API </reference/v2/payments-api/overview>` or 
   via the `Mollie mobile app <https://www.mollie.com/products/mobile>`_. This type of QR code should only be used for checkout and in-store scenarios. See more information below on the 
   supported methods and how a consumer should use the QR-code.
...
#. A payment link QR code. This QR code stays valid for a longer period and can be used for any method available at Mollie. These methods should be activated in your Mollie account under 'profiles'. 
   The advice though here is to only use mobile friendly methods, such as Paypal, Bancontact and Ideal. This can be created by creating a payment link via the :doc:`payment link API <//reference/v2/payment-links-api/overview>` 
   and converting this ``paymentLink`` URL to a QR code via a link to QR code tool/SDK. The expiry time of the QR-code can be defined via the ``expiresAt`` parameter. 
